...
data=se.selectwhere("shopaddress", "shopid", shopid);
%>               
<div id="locationField">

  Country/State/City:<br><input id="autocomplete" placeholder="City,State,Country"
   ...
   <table id="address">
      <tr>
        ...
        <td class="slimField"><input class="field" id="street_number"
              disabled="true" value="<%if(data.get(3)!=null){%><%=data.get(3) %><%}%>"></input></td>
              <td class="label">Road No</td>
        <td class="wideField" colspan="2"><input class="field" id="route"
              disabled="true" value="<%if(data.get(4)!=null){%><%=data.get(4) %><%}%>"></input></td>
      </tr>
      <tr>
        <td class="label">City</td>
        <td class="wideField" colspan="3"><input class="field" id="locality"
              disabled="true" value="<%if(data.get(5)!=null){%><%=data.get(5) %><%}%>"></input></td>
      </tr>
      <tr>
        <td class="label">State</td>
        <td class="slimField"><input class="field"
              id="administrative_area_level_1" value="<%if(data.get(6)!=null){%><%=data.get(6) %><%}%>" disabled="true"></input></td>
        <td class="label">Zip code</td>
        <td class="wideField"><input class="field" id="postal_code"
              disabled="true" value="<%if(data.get(7)!=null){%><%=data.get(7) %><%}%>"></input></td>
      </tr>
      <tr>
        <td class="label">Country</td>
        <td class="wideField" colspan="3"><input class="field"
              id="country" disabled="true" value="<%if(data.get(8)!=null){%><%=data.get(8) %><%}%>"></input></td>
      </tr>
    </table>
Land Marks:<textarea id="landmark" ><%if(data.get(9)!=null){%><%=data.get(9) %><%}%></textarea>
<input type="text" id="shopid" value="<%if(data.get(10)!=null){%><%=data.get(10) %><%}%>" />
